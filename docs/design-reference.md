...
- Match Claude Code-style behaviors: inline commands (`/init`, `/config`, `/exit`), `@file` references with completion, `.claude` tool loading, visible todo list, and rich progress output. Doc rules live in `.dogent/dogent.md`.
...
- `.dogent/dogent.md`: project writing guidelines produced by `/init`; users can edit directly in `.dogent/`.
- `.dogent/dogent.json`: local runtime config produced by `/config`.
- `.dogent/memory.md`: transient scratchpad for ideas/notes; cleaned when tasks complete.
...
- Document Orchestrator: coordinates planning, research, drafting per section, validation, polishing, and citation assembly; manages `.dogent/memory.md` lifecycle.
...
- System prompt: use Dogent’s own system prompt (no Claude Code preset text), focused on document writing and the project rules; keep it fixed. Per-turn user prompts append dynamic todo state and resolved `@file` references.
  ...
  - Streaming: set `include_partial_messages=True` to surface `StreamEvent` tokens into the Rich UI; use `receive_response` for concurrent render and `interrupt()` to stop long actions.
  - Safety: rely on SDK defaults for spend limits; use `sandbox` for bash isolation, `cwd` pinned to user directory, `env` for model/base URL overrides, `extra_args` for CLI flags.
...
9) Cleanup: clear `.dogent/memory.md` when temporary notes are no longer needed.

## Prompt Template Design (stored under `doc/prompts/`)
- `system.md`: establishes role, safety, command semantics, tool usage rules, CWD scope, Chinese Markdown default, adherence to `.dogent/dogent.md` doc rules; uses Dogent’s own system prompt (no Claude Code preset).
...
- Hooks: wire `PreToolUse`, `PostToolUse`, `UserPromptSubmit`, `PreCompact`, `Stop/SubagentStop` to enforce guardrails (e.g., block dangerous bash), inject context (todo state), or halt on errors.
  - Streaming UX: enable `include_partial_messages` to surface `StreamEvent` for token-by-token UI; use `receive_response` for concurrent display and `interrupt()` for user-initiated stop.
- Session control: support `resume`/`fork_session`, `max_turns`, and other SDK-provided controls when available.
