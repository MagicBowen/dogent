...
- [done] Add `.gitignore` entries for `.doc-config.yaml`, `.memory.md`, `/images/`, `__pycache__`, `.DS_Store`, and build artifacts.

## 2) Config and Guidelines
- [done] Implement `config.py`: load env vars, merge `.doc-config.yaml`, defaults (language/format/image_dir/max_section_tokens/research_provider), validation for required token/model, add config to `.gitignore`, flags for web/fs tool allow.
- [done] Implement `guidelines.py`: ensure `.claude.md` template; load and summarize rules.
...
- [done] Implement `runtime.py`: build `ClaudeAgentOptions` with explicit tools (`Read`/`Write`/`Edit`/`MultiEdit`/`WebSearch`/`WebFetch`), Dogent system prompt, model/fallback handling, env overrides, cwd, partial streaming, setting_sources, max budget.
...
- [done] Implement `images.py` (download to images/), `citations.py` (collect/render), `memory.py` (scratchpad lifecycle).
...
- [done] Add tests: config precedence, guidelines template, @reference resolution, runtime fallback handling.
