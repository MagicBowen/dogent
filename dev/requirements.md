...
- The default value of web_profile is `default` as it does now, and the default value of vision_profile should be `none` (so that when the user uses `@image` to reference image, an error will be reported to the user, following the fail-fast principle).
- When the setting of vision_profile is `none`，should disable the related mcp tools for agent.
